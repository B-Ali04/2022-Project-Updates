Select
    ...
    case
      ...
      when SHRDGMR.SHRDGMR_GRAD_DATE is not null 
           and (exists(select * from SFRWDRL where  SFRWDRL.SFRWDRL_PIDM = SPRIDEN.SPRIDEN_PIDM 
           and  SFRWDRL.SFRWDRL_ESTS_CODE in ('WS', 'AW','MW') 
                )
           ) then ('* '|| to_char(SHRDGMR.SHRDGMR_GRAD_DATE, 'MM/DD/YYYY'))
      when (exists(select * from SFRWDRL where SFRWDRL.SFRWDRL_PIDM = SPRIDEN.SPRIDEN_PIDM 
                          and SFRWDRL.SFRWDRL_ESTS_CODE in ('WS', 'AW','MW'))) 
